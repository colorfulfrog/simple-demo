...
public class OSSManageUtil {
	...
	public static String uploadFile(InputStream file,long contentLength,String fileName) throws Exception{
		...
		String remoteFilePath = Constants.testBucket.replaceAll("\\\\","/")+"/";
		//创建上传Object的Metadata
		...
		//上传文件
		ossClient.putObject(Constants.bucketName, remoteFilePath + fileName, file, objectMetadata);
		return Constants.osswebsite+Constants.testBucket+"/"+ fileName;
	}
	...
	public static String imageFileUpload(byte[] fileByte, String user_avatar) throws Exception {  
		...
		try {  
			...
			OSSClient client = new OSSClient(Constants.endpoint, Constants.accessKeyId, Constants.accessKeySecret);
			ensureBucket(client, bucketName);  
			ObjectMetadata objectMeta = new ObjectMetadata();  
			objectMeta.setContentLength(fileByte.length);  
			client.putObject(bucketName, fileName, input, objectMeta);  
			String saveUrl = Constants.endpoint+"/"+Constants.bucketName+"/"+ fileName;  
			return saveUrl;  
		} catch (Exception e) {  
			e.printStackTrace();  
			return null;  
	...
	public static void main(String[]st)throws Exception{
		...
		//		 deleteFile("http://oss-cn-shenzhen.aliyuncs.com", "TJXWCe9Jdc1SKQgc", "llGUl8zErQzLrKRtHUDQMM0UwiZgXl", "b04ed157-9fed-4310-b728-6387759da5b6", "app/head/center_bg_02.jpg");
	}
}
