package com.yxcoach.common.base.easypoi.excel.xml;
...
import com.yxcoach.common.base.easypoi.excel.ExcelDefinitionReader;
import com.yxcoach.common.base.easypoi.excel.config.ExcelConfig;
...
import com.yxcoach.common.base.easypoi.excel.exception.ExcelException;
import com.yxcoach.common.base.easypoi.excel.parsing.CellValueConverter;
import com.yxcoach.common.base.easypoi.util.ReflectUtil;
...
public class XMLExcelDefinitionReader implements ExcelDefinitionReader {
	 ...
	 * @throws Exception
	 */
	public XMLExcelDefinitionReader(String location, Map<String, Object> formatMap) throws Exception {
	...
	private void init(String location) throws Exception {
		...
		InputStream is = null;
		if (location.startsWith(ExcelConfig.tempPath)) {
			is = new FileInputStream(location);
		} else {
			Resource resource = new ClassPathResource(location);
			is = resource.getInputStream();
		}
		loadExcelDefinitions(is);
	}
	...
	protected void processExcelDefinition(Element ele) {
		...
		try {
			...
			throw new ExcelException(
					"Excel 配置文件[" + location + "] , id为 [" + id + "] 的 class 为 [" + className + "] 类不存在 ");
		}
		...
		if (StringUtils.isNotBlank(type)) {
			excelDefinition.setType(type);
		}
		processField(ele, excelDefinition);
		/******** 添加自定义字段的追加 start ******************/
		if (null != formatMap && null != formatMap.get(id + "_custom_field")) {
			// 自定义字段
			List<FieldValue> fiels = excelDefinition.getFieldValues();
			List<FieldValue> customFiels = (List<FieldValue>) formatMap.get(id + "_custom_field");
			if (!customFiels.isEmpty()) {
				fiels.addAll(customFiels);
				excelDefinition.setFieldValues(fiels);
			}
		}
		/******** 添加自定义字段的追加 end ******************/
		...
		if (fieldNode != null) {
			for (int i = 0; i < fieldNode.getLength(); i++) {
				...
				if (node instanceof Element) {
					...
					Validate.isTrue(StringUtils.isNotBlank(name), "Excel 配置文件[" + location + "] , id为 [ "
							+ excelDefinition.getId() + " ] 的 name 属性不能为 [ null ]");
					fieldValue.setName(name);
					String title = fieldEle.getAttribute("title");
					Assert.isTrue(StringUtils.isNotBlank(title), "Excel 配置文件[" + location + "] , id为 [ "
							+ excelDefinition.getId() + " ] 的 title 属性不能为 [ null ]");
					...
					if (StringUtils.isNotBlank(format)) {
						if (null != formatMap && null != formatMap.get(format) && "" != formatMap.get(format)) {
							fieldValue.setFormat(formatMap.get(format).toString());
						} else {
					...
					String maxLength = fieldEle.getAttribute("maxLength");
					if (StringUtils.isNotBlank(maxLength)) {
						fieldValue.setMaxLength(Integer.valueOf(maxLength));
					}
					...
					if (StringUtils.isNotBlank(align)) {
						try {
							// 获取cell对齐方式的常量值
							short constValue = ReflectUtil.getConstValue(CellStyle.class,
									"ALIGN_" + align.toUpperCase());
					...
					if (StringUtils.isNotBlank(titleBgColor)) {
						try {
							// 获取cell对齐方式的常量值
							IndexedColors color = ReflectUtil.getConstValue(IndexedColors.class,
									titleBgColor.toUpperCase());
							fieldValue.setTitleBgColor(color.index);
						} catch (Exception e) {
							throw new ExcelException("Excel 配置文件[" + location + "] , id为 [ " + excelDefinition.getId()
									+ " ] 的 titleBgColor 属性不能为 [ " + titleBgColor
									+ " ],具体看[org.apache.poi.ss.usermodel.IndexedColors]支持的颜色");
						}
					}
					// cell标题字体颜色
					...
					if (StringUtils.isNotBlank(titleFountColor)) {
						try {
							// 获取cell对齐方式的常量值
							IndexedColors color = ReflectUtil.getConstValue(IndexedColors.class,
									titleFountColor.toUpperCase());
							if (color == null) {
								throw new ExcelException("Excel 配置文件[" + location + "] , id为 [ "
										+ excelDefinition.getId() + " ] 的 titleFountColor 属性不能为 [ " + titleFountColor
										+ " ],具体看[org.apache.poi.ss.usermodel.IndexedColors]支持的颜色");
							}
					...
					// 导出时是否隐藏
					String hidden = fieldEle.getAttribute("hidden");
					if (StringUtils.isNotBlank(hidden)) {
						fieldValue.setHidden(Boolean.parseBoolean(hidden));
					}
					// 下拉选择
					String selectValue = fieldEle.getAttribute("selectValue");
					if (StringUtils.isNotBlank(selectValue)) {
						fieldValue.setSelectValue(selectValue);
					}
					...
					if (StringUtils.isNotBlank(cellValueConverterName)) {
						try {
							...
							if (!CellValueConverter.class.isAssignableFrom(clazz)) {
								throw new ExcelException("配置的：" + cellValueConverterName + "错误,不是一个标准的["
										+ CellValueConverter.class.getName() + "]实现");
							}
					...
					if (StringUtils.isNotBlank(roundingMode)) {
						try {
							// 获取roundingMode常量值
							RoundingMode mode = ReflectUtil.getConstValue(RoundingMode.class,
									roundingMode.toUpperCase());
							if (mode == null) {
								throw new ExcelException("Excel 配置文件[" + location + "] , id为 [ "
										+ excelDefinition.getId() + " ] 的 roundingMode 属性不能为 [ " + roundingMode
										+ " ],具体看[java.math.RoundingMode]支持的值");
							}
